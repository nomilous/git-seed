...
program.command('pull').description('[PENDING] Git pull across all nested git repos not up-to-date with .nez_tree').action(function() {
  return GitAction.assign(program).pull(arguments);
});

program.command('push').description('[PENDING] Git push across all nested git repos pending commits and update .nez_tree').action(function() {
