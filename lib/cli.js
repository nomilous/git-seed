...
var GitAction, colors, noNotify, program;
...
noNotify = function(status) {
  return console.log(status.context.bold, status.message);
};

program.command('init').description('Assemble the initial .nez_tree control file into [root]').action(function() {
  var error, success;
  return GitAction.configure(program, success = function() {}, error = function() {}, noNotify).init(arguments);
});

program.command('status').description('Git status across all nested git repos').action(function() {
  var error, success;
  return GitAction.configure(program, success = function() {}, error = function() {}, noNotify).status(arguments);
});
...
program.command('commit').description('Git commit across all nested repos with staged changes.').action(function() {
  var error, success;
  return GitAction.configure(program, success = function() {}, error = function() {}, noNotify).commit(arguments);
});

program.command('pull').description('Git pull across all nested git repos.').action(function() {
  var error, success;
  return GitAction.configure(program, success = function() {}, error = function() {}, noNotify).pull(arguments);
});

program.command('push').description('[PENDING] Git push across all nested git repos (per .git-seed)').action(function() {
  var error, success;
  return GitAction.configure(program, success = function() {}, error = function() {}, noNotify).push(arguments);
});
