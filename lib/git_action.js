...
module.exports = GitAction = {
  ...
  init: function() {
    ...
    GitAction.error = '';
    try {
      return GitTree.init(GitAction.root);
    } catch (error) {
  ...
  clone: function() {
    ...
    GitAction.error = '';
    return (new GitTree(GitAction.root)).clone(function(error, result) {
      if (error) {
        console.log('(error) '.red + error.toString());
        process.exit(4);
      }
  ...
  commit: function() {
    ...
    GitAction.error = '';
    return (new GitTree(GitAction.root)).commit(GitAction.message, function(error, result) {
      if (error) {
        console.log('(error) '.red + error.toString());
        process.exit(5);
      }
