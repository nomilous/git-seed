...
module.exports = GitAction = {
  ...
  init: function() {
    ...
    GitAction.deferral.notify.info.good('start seed init', "recurse for git repositories in '" + GitAction.root + "'");
    GitAction.error = '';
    if (!GitAction.gotDirectory(GitAction.root + '/.git')) {
      GitAction.deferral.notify.info.bad('missing root repo', "no git reposititory in '" + GitAction.root + "'");
      return;
    }
  ...
  status: function() {
    ...
    GitAction.deferral.notify.info.good('start seed status', "for all git repositories in '" + GitAction.root + "/.git-seed'");
    GitAction.error = '';
    return (new GitSeed(GitAction.root, GitAction.plugin, GitAction.deferral)).status();
  },
  clone: function() {
    if (typeof GitAction.deferral === 'undefined') {
      throw new Error('configure() was not called');
    }
    GitAction.deferral.notify.info.good('start seed clone', "for all git repositories in '" + GitAction.root + "/.git-seed'");
    GitAction.error = '';
    return (new GitSeed(GitAction.root, GitAction.plugin, GitAction.deferral)).clone();
  },
  commit: function() {
    if (typeof GitAction.deferral === 'undefined') {
      throw new Error('configure() was not called');
    }
    GitAction.deferral.notify.info.good('start seed commit', "for any git repositories with staged changes in '" + GitAction.root + "/.git-seed' ");
    GitAction.error = '';
    if (!GitAction.message) {
      GitAction.deferral.notify.info.bad('missing commit message', 'use -m "message"');
      return;
    }
    return (new GitSeed(GitAction.root, GitAction.plugin, GitAction.deferral)).commit(GitAction.message);
  },
  pull: function() {
    if (typeof GitAction.deferral === 'undefined') {
      throw new Error('configure() was not called');
    }
    GitAction.deferral.notify.info.good('start seed pull', "for all git repositories in '" + GitAction.root + "/.git-seed'");
